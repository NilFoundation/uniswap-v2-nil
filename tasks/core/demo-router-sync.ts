...
task("demo-router-sync", "Run demo with Uniswap Router with Sync methods").setAction(
  async (taskArgs, hre) => {
    ...
    const hash = await wallet.sendMessage({
      ...
      data: encodeFunctionData({
        ...
        args: [pairAddress, walletAddress, mintCurrency0Amount, mintCurrency1Amount, 1, 1],
      }),
      tokens: [
        {
    ...
    const hash2Receipt = (await publicClient.getMessageReceiptByHash(hash2))!;
    console.log(`hash2: ${hash2}`);
    console.log(`hash2Receipt: ${JSON.stringify(hash2Receipt, (key, value) => typeof value === 'bigint' ? value.toString() : value)}`);
