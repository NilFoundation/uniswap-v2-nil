...
.setAction(async (taskArgs, hre) => {
  ...
  const pairBalanceToken0 = await token0Contract.getCurrencyBalanceOf(pairAddress.toLowerCase());
  const pairBalanceToken1 = await token1Contract.getCurrencyBalanceOf(pairAddress.toLowerCase());
  ...
  let userBalanceToken0 = await token0Contract.getCurrencyBalanceOf(walletAddress);
  let userBalanceToken1 = await token1Contract.getCurrencyBalanceOf(walletAddress);
  ...
  await user.sendCurrencyPublic(pairAddress.toLowerCase(), lpAddress, userLpBalance);
  ...
  const balanceToken0 = await token0Contract.getCurrencyBalanceOf(pairAddress.toLowerCase());
  const balanceToken1 = await token1Contract.getCurrencyBalanceOf(pairAddress.toLowerCase());
  ...
  console.log("User Balance token0 after burn:", userBalanceToken0.toString());
  console.log("User Balance token1 after burn:", userBalanceToken1.toString());
