...
.setAction(async (taskArgs, hre) => {
  ...
  const pairCurrency0Balance = await currency0.getCurrencyBalanceOf(pairAddress);
  console.log("Pair Balance 0:", pairCurrency0Balance.toString());

  const pairCurrency1Balance = await currency1.getCurrencyBalanceOf(pairAddress);
  ...
  console.log(`Reserves - Currency0: ${reserve0.toString()}, Currency1: ${reserve1.toString()}`);
