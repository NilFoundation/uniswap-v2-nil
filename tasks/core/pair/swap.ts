...
import { Currency, UniswapV2Pair, UserWallet } from "../../../typechain-types";
...
.setAction(async (taskArgs, hre) => {
  ...
  const currency0Contract = CurrencyFactory.attach(currency0Address) as Currency;
  const currency1Contract = CurrencyFactory.attach(currency1Address) as Currency;
  ...
  console.log("Expected output amount for swap:", expectedOutputAmount.toString());

  // Log balances before the swap
  const balanceCurrency0Before = await currency0Contract.getCurrencyBalanceOf(userWalletAddress);
  const balanceCurrency1Before = await currency1Contract.getCurrencyBalanceOf(userWalletAddress);
  console.log("Balance of currency0 before swap:", balanceCurrency0Before.toString());
  console.log("Balance of currency1 before swap:", balanceCurrency1Before.toString());

  // Attach to the UserWallet contract
  const UserWalletFactory = await hre.ethers.getContractFactory("UserWallet");
  const userWallet = UserWalletFactory.attach(userWalletAddress) as UserWallet;
  ...
  const balanceCurrency0After = await currency0Contract.getCurrencyBalanceOf(userWalletAddress);
  const balanceCurrency1After = await currency1Contract.getCurrencyBalanceOf(userWalletAddress);
  console.log("Balance of currency0 after swap:", balanceCurrency0After.toString());
  console.log("Balance of currency1 after swap:", balanceCurrency1After.toString());
});
