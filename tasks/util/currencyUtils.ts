import { ExternalMessageEnvelope, bytesToHex, hexToBytes, toHex, waitTillCompleted } from "@nilfoundation/niljs";
 ...
 */
function createExternalMessage({ contractAddress, chainId, seqNo, functionName, args, abi }) {
...
async function sendAndAwait(publicClient, walletAddress, message) {
	...
	await waitTillCompleted(publicClient, shardNumber(walletAddress), toHex(message.hash()));
}

/**
 * Function to withdraw from a faucet to a specific address.
 */
export async function faucetWithdrawal(address, value = 1000000000000000000n, faucetAddress, hre, client) {
