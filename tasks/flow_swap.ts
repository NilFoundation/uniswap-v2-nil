...
task("flow_swp", "Init pair, mint tokens and run swap")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const factory = Factory.attach(
      taskArgs.factory.toLowerCase(),
    ) as UniswapV2Factory;

    // Create pair
    let pairAddress = await factory.getTokenPair(
    ...
    pairAddress = (
      await factory.getTokenPair(token0Address, token1Address)
    ).toLowerCase();
    ...
    console.log(
      "User wallet pair token balance " +
        (await pair.getCurrencyBalanceOf(walletAddress)),
    );
    ...
    console.log("Swapped");
    const balanceToken0After =
      await token0Contract.getCurrencyBalanceOf(walletAddress);
    const balanceToken1After =
      await token1Contract.getCurrencyBalanceOf(walletAddress);
