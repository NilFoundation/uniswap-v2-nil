...
import type { Token, UniswapV2Pair } from "../typechain-types";
...
task("swap", "Swap token0 to token1")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const balanceToken0Before =
      await Token0.getCurrencyBalanceOf(walletAddress);
    const balanceToken1Before =
      await Token1.getCurrencyBalanceOf(walletAddress);
    ...
    const pairBalance0 = await Token0.getCurrencyBalanceOf(
      pairAddress.toLowerCase(),
    );
    const pairBalance1 = await Token1.getCurrencyBalanceOf(
      pairAddress.toLowerCase(),
    );
    ...
    console.log(
      `Swapping... token0 ${0} and token1 ${expectedOutputAmount} to ${walletAddress}`,
    );
    try {
      ...
      if (error?.error?.data?.message) {
        console.error(
          "Swap failed with revert reason:",
