...
.setAction(async (taskArgs, hre) => {
	...
	const currency0 = CurrencyFactory.attach(taskArgs.currency0) as Currency;
	const currency1 = CurrencyFactory.attach(taskArgs.currency1) as Currency;

	// Mint and send Currency0
	console.log(`Minting ${mintAmount} Currency0 to wallet ${taskArgs.wallet}...`);
	await currency0.mintCurrencyPublic(mintAmount);
	await currency0.sendCurrencyPublic(taskArgs.wallet, await currency0.getCurrencyId(), mintAmount);

	// Mint and send Currency1
	console.log(`Minting ${mintAmount} Currency1 to wallet ${taskArgs.wallet}...`);
	await currency1.mintCurrencyPublic(mintAmount);
	await currency1.sendCurrencyPublic(taskArgs.wallet, await currency1.getCurrencyId(), mintAmount);

	// Verify the balance of the recipient wallet for both currencies
	const recipientBalanceCurrency0 = await currency0.getCurrencyBalanceOf(taskArgs.wallet);
	const recipientBalanceCurrency1 = await currency1.getCurrencyBalanceOf(taskArgs.wallet);
