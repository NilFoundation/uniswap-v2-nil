...
.setAction(async (taskArgs, hre) => {
  ...
  const currencyContract = CurrencyFactory.attach(currencyAddress) as Currency;
  ...
  if (currentBalance < amount) {
    ...
    console.log(`Insufficient balance. Minting ${amountNeeded} more currency...`);
  ...
  await currencyContract.sendCurrencyPublic(recipientAddress, currencyId, amount);
  ...
  const recipientBalance = await currencyContract.getCurrencyBalanceOf(recipientAddress);
